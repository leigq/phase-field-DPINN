...
class PINN:
    ...
    def train(self, epoch):
        ...
        for i in range(epoch):
            ...
            if i%2000==0:
                with torch.no_grad():
                    torch.save(self.model.state_dict(), f'DKAN_AV_5e_3/DKAN_AV_5e_3_{i}.pth')
                    ...
                    plt.tight_layout()
                    # plt.show()
                    plt.savefig(f'DKAN_AV_5e_3/DKAN_AV_5e_3_{i}.png')
                    plt.close()
        torch.save(self.model.state_dict(), "DKAN_AV_5e_3/DKAN_AV_5e_3_end.pth")
        scipy.io.savemat(f'DKAN_AV_5e_3/DKAN_AV_5e_3_history.mat', {'loss': self.log['loss'],'loss_ac_ch': self.log['loss_ac_ch'],'loss_bc': self.log['loss_bc'],'loss_ic': self.log['loss_ic'],'AV': self.log['AV']})
