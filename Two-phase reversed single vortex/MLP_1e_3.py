...
class PINN:
    def __init__(self, device='cuda', seed=42):
        ...
        self.model = DNN(3, 1, 128, 10, act='tanh').to(device)
    ...
    def Msef(self,train_data_pde_use):
        xi = torch.tensor(1e-3); M0 = torch.tensor(1e-4)
    ...
    def train(self, epoch):
        ...
        for i in range(epoch):
            ...
            if i%100==0:
                print(f'{i}|{epoch} loss={loss.item():.4e} CH={loss_pde_ch.item():.4e} BC={loss_bc.item():.4E} IC={loss_ic.item():.4e} sensor=0')
            ...
            if i%500==0:
                with torch.no_grad():
                    torch.save(self.model.state_dict(), f'MLP_1e_3/MLP_1e_3_{i}.pth')
                    ...
                    plt.tight_layout()
                    # plt.show()
                    plt.savefig(f'MLP_1e_3/MLP_1e_3_{i}.png')
                    plt.close()
        torch.save(self.model.state_dict(), "MLP_1e_3/MLP_1e_3_end.pth")
        scipy.io.savemat(f'MLP_1e_3/MLP_1e_3_history.mat', {'loss': self.log['loss'],'loss_ac_ch': self.log['loss_ac_ch'],'loss_bc': self.log['loss_bc'],'loss_ic': self.log['loss_ic'],'AV': self.log['AV']})
