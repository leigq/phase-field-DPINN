...
class PINN:
    ...
    def Msef(self,train_data_pde_use):
        xi = torch.tensor(1e-2); M0 = torch.tensor(1e-4)
    ...
    def train(self, epoch):
        ...
        for i in range(epoch):
            ...
            if i%500==0:
                with torch.no_grad():
                    torch.save(self.model.state_dict(), f'MLP_1e_2/MLP_1e_2_{i}.pth')
                    ...
                    plt.tight_layout()
                    # plt.show()
                    plt.savefig(f' /MLP_1e_2_{i}.png')
                    plt.close()
        torch.save(self.model.state_dict(), "MLP_1e_2/MLP_1e_2_end.pth")
        scipy.io.savemat(f'MLP_1e_2/MLP_1e_2_history.mat', {'loss': self.log['loss'],'loss_ac_ch': self.log['loss_ac_ch'],'loss_bc': self.log['loss_bc'],'loss_ic': self.log['loss_ic'],'AV': self.log['AV']})
