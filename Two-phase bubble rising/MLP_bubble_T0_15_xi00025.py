...
#model=torch.load('MLP_bubble_T0_15_xi00025/bubble_t0_15_40000.pt',map_location='cuda')
...
#此处先将网络参数导入
optimizer =  torch.optim.AdamW(model.parameters(),lr=5e-3,weight_decay=1e-8)
...
for epoch in range(40001):
    ...
    if np.mod(epoch,1000)==0:
        with torch.no_grad():
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig(f'MLP_bubble_T0_15_xi00025/bubble_{epoch}.png', dpi=300)
            plt.close()
            torch.save(model.module, f'MLP_bubble_T0_15_xi00025/bubble_t0_15_{epoch}.pt')
