...
t_start=1.5
t_end=3.0
model=torch.load('MLP_bubble_T0_15_xi001_refine/bubble_t0_15_40000.pt',map_location='cuda').to(device)
...
t_vals = np.linspace(t_start, t_end, 4)  # 101 points in the t direction
...
t_init = torch.ones_like(x_init) * t_start
print('create initial condition')
with torch.no_grad():
    Output_start = model(torch.cat([x_init, y_init, t_init], dim=1))
    C_init = Output_start[:, 0:1]
    u_init = Output_start[:, 1:2]
    v_init = Output_start[:, 2:3]

x_init = torch.tensor(x_init, requires_grad=True).float().to(device)
y_init = torch.tensor(y_init, requires_grad=True).float().to(device)
x_init = torch.reshape(x_init, [num_x * num_y, 1])
y_init = torch.reshape(y_init, [num_x * num_y, 1])
...
scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=40001, eta_min=1e-6)
...
for epoch in range(40001):
    ...
    t0_init = torch.ones([N_init, 1]).float().to(device) * t_start
    ...
    t_cyc = (t_end - t_start) * torch.rand_like(x_cyc_up, requires_grad=True).float().to(device) + t_start
    ...
    if np.mod(epoch,2000)==0:
        with torch.no_grad():
            ...
            titles_phi = [r'$\phi,\ t=2.0$', r'$\phi,\ t=2.5$', r'$\phi,\ t=3.0$']
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig(f'MLP_bubble_T15_3_xi001/bubble_{epoch}.png', dpi=300)
            plt.close()
            torch.save(model.module, f'MLP_bubble_T15_3_xi001/bubble_t15_3_{epoch}.pt')

# torch.save(model.module, f'Bubble_MLP_T01_v1/bubble_t01_end.pt')
