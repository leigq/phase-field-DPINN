...
t_start=1.5
t_end=3.0
model=torch.load('DKAN_bubble_T0_15_xi0005/bubble_t01_40000.pt',map_location='cuda').to(device)
model = nn.DataParallel(model)
...
for epoch in range(40001):
    ...
    if np.mod(epoch,1000)==0:
        with torch.no_grad():
            ...
            plt.tight_layout()
            # plt.show()
            plt.savefig(f'DKAN_bubble_T15_3_xi0005/bubble_{epoch}.png', dpi=300)
            plt.close()
            torch.save(model.module, f'DKAN_bubble_T15_3_xi0005/bubble_t01_{epoch}.pt')
