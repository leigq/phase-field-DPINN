...
class PINN:
    def __init__(self, device='cuda', seed=42):
        ...
        self.t_start_second = 4e-2
        self.t_end_second  = 8e-2
        ...
        self.model = torch.load('DKAN_ICE898_T25_T0_4/droplet_40000.pt').to(device)
    ...
    def train(self, epoch):
        ...
        for i in range(epoch):
            ...
            if i % 1000 == 0:
                torch.save(self.model.module, f'DKAN_ICE898_T25_T4_8/droplet_{i}.pt')
                with torch.no_grad():
                    ...
                    # fig.tight_layout()
                    # plt.show()
                    plt.savefig(f'DKAN_ICE898_T25_T4_8/droplet_{i}.png', dpi=300)
                    plt.close()
