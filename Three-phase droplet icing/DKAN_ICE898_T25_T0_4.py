...
class PINN:
    def __init__(self, device='cuda', seed=42):
        ...
        self.t_start_second = 0.0
        self.t_end_second  = 4e-2
        ...
        self.model = DNN(3,5,129,5,self.T_sub,X_all=torch.tensor([[0, self.x_start, self.y_start], [2e2, self.x_end, self.y_end]], device=device)).to(device)
        # self.model.load_state_dict(torch.load("DKAN_ICE900_u0_T3_6_old_model_params.pth"), strict=False)
    ...
    def sample_data0(self):  # 无量纲化时间空间
        ...
        with torch.no_grad():
            ...
            th = np.random.uniform(0, 2 * np.pi, 20000)
            rr = self.r0 * (1 + 0.05 * np.random.uniform(-1, 1, 20000))  # ±5%窄带
            Xc = self.x0 + rr * np.cos(th)
            Yc = self.y0 + rr * np.sin(th)
            m = (Yc >= self.y_start)
            y_limit = self.y_start + 0.02 * (self.y_end - self.y_start)
            xlimits_initial = np.array([[self.x_start, self.x_end], [self.y_start, y_limit]])
            lhs_left = LHS(xlimits=xlimits_initial, random_state=self.seed)
            initial_dense = lhs_left(20000)
            P = np.vstack((np.c_[x_grid[:, :, 0].ravel(), y_grid[:, :, 0].ravel()], np.c_[Xc[m], Yc[m]], initial_dense))
            P = np.unique(P, axis=0)
            self.train_data_ic = np.c_[np.ones(len(P)) * self.t_start.item(), P]
            self.train_data_ic = torch.tensor(self.train_data_ic, dtype=torch.float32).to(device)

            th = np.random.uniform(0, 2 * np.pi, 20000)
            rr = self.r0 * (0.75 + 0.25 * np.random.uniform(-1, 1, 20000))  # ±5%窄带
            Xc = self.x0 + rr * np.cos(th)
            Yc = self.y0 + rr * np.sin(th)
            m = (Yc >= self.y_start)
            P = np.c_[Xc[m], Yc[m]]
            ...
            # Boundary conditions
            bc_left_base = np.vstack([t_grid[:, 0, :].ravel(), x_grid[:, 0, :].ravel(), y_grid[:, 0, :].ravel()]).T  # t in [0, 1], x = 0
            xlimits_left = np.array([[self.t_start, self.t_end], [self.x_start, self.x_start], [self.y_start, y_limit]])
            lhs_left = LHS(xlimits=xlimits_left, random_state=self.seed)
            bc_left_dense = lhs_left(20000)
            self.train_data_bc_left = np.unique(np.vstack([bc_left_base, bc_left_dense]), axis=0)
    ...
    def Mseb(self, train_data_pde_use_left,train_data_pde_use_right,
             train_data_ic_circle, train_data_random_circle,
             train_data_bc_left, train_data_bc_right, train_data_ic, train_data_bc_up, train_data_bc_down):

        pred_0 = self.model(train_data_ic_circle)[:,4:5]
        pred_1 = self.model(train_data_random_circle)[:,4:5]
        loss_circle=0.01*(pred_0-pred_1).pow(2).mean()

        T_min = (273.15 + self.T_sub) / self.T_ref; T_max = (273.15 + 25) / self.T_ref
        ...
        loss_down = (v.pow(2).mean()+5.0*(T-(273.15+self.T_sub)/self.T_ref).pow(2).mean()+(c+1).pow(2).mean()+(phi-phi_down).pow(2).mean())/4.0

        loss_bc = loss_left_right + loss_up + loss_down

        T_exact = torch.where(train_data_ic[:, 2:3] <= self.y_start, T_min, T_max)
        c_exact = torch.where(train_data_ic[:, 2:3] <= self.y_start, -1.0, 0.0)
        phi_exact = torch.where((train_data_ic[:,1:2]**2+(train_data_ic[:,2:3]-self.y0)**2)<=self.r0**2, 1.0,-1.0)

        pred = self.model(train_data_ic)
        v = pred[:,0:1]; p = pred[:, 1:2]; T = pred[:, 2:3]; c = pred[:, 3:4]; phi = pred[:, 4:5];
        loss_ic=(v.pow(2).mean()+(T-T_exact).pow(2).mean()+(c-c_exact).pow(2).mean()+(phi-phi_exact).pow(2).mean())/4.0
        return (loss_circle + loss_sym + loss_bc + loss_ic)/6.0
    ...
    def train(self, epoch):
        ...
        optimizer = torch.optim.AdamW(self.model.parameters(), lr=0.00005, weight_decay=1e-8)
        ...
        for i in range(epoch):
            ...
            loss_bc_ic = self.Mseb(train_data_pde_use_left,train_data_pde_use_right,
            ...
            if i % 1000 == 0:
                torch.save(self.model.module, f'DKAN_ICE898_T25_T0_4/droplet_{i}.pt')
                with torch.no_grad():
                    ...
                    # fig.tight_layout()
                    # plt.show()
                    plt.savefig(f'DKAN_ICE898_T25_T0_4/droplet_{i}.png', dpi=300)
                    plt.close()
