...
class PINN:
    def __init__(self, device='cuda', seed=42):
        ...
        self.t_start_second = 12e-2
        self.t_end_second  = 14e-2
        ...
        self.model = torch.load('DKAN_ICE898_T25_T8_12/droplet_30000.pt').to(device)
    ...
    def Mseb(self, train_up_phi, train_data_pde_use_left,train_data_pde_use_right,
        ...
        loss_circle=(pred_0-pred_1).pow(2).mean()
    ...
    def train(self, epoch):
        ...
        for i in range(epoch):
            ...
            if i % 1000 == 0:
                torch.save(self.model.module, f'DKAN_ICE898_T25_T12_14/droplet_{i}.pt')
                with torch.no_grad():
                    ...
                    # fig.tight_layout()
                    # plt.show()
                    plt.savefig(f'DKAN_ICE898_T25_T12_14/droplet_{i}.png', dpi=300)
                    plt.close()
